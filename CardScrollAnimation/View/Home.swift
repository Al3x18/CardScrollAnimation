...
struct Home: View {
    ...
    var body: some View {
        ScrollView(.vertical) {
            VStack(spacing: 0) {
                ...
                .padding(15)
                /// Hide content behind Balance Card
                .mask {
                    Rectangle()
    ...
    /// Marked as `nonisolated` because this helper is pure and does not touch UI or actor-isolated state.
    /// SwiftUI may execute the closures in `.visualEffect`/`GeometryReader` from a nonisolated context.
    /// Without `nonisolated`, instance methods inside a `View` are treated as main-actor isolated,
    /// which triggered a warning when this function was called from those closures.
    /// Declaring it `nonisolated` makes it callable from any actor without hopping to the main actor,
    /// and is safe here since it only computes a value from the provided `GeometryProxy`.
    ...
    func CardView(_ card: Card) -> some View {
        GeometryReader {
            ...
            ZStack {
                ...
                .padding(15)
                .offset(y: progress * -25) // Move balance up with scrolling
            }
            .offset(y: -offset)
    ...
    func ExpenseCardView(_ expense: Expense) -> some View {
     ...
     ///
     /// Whenever the dragging ends in the scrollview, check if the target offset is above 75 or not.
     /// If it's above 75, then no change is required, if it's below 75 reset the scroll offset to zero,
     /// with this the scroll look good even when the dragging ends in the middle of the transition
